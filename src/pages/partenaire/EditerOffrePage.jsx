...
export default function EditerOffrePage() {
    ...
    const load = async () => {
      try {
        ...
        if (forfaitsRes.status === 'fulfilled') {
          forfaits = Array.isArray(forfaitsRes.value.data) ? forfaitsRes.value.data : []
        ...
        if (offreRes.status === 'fulfilled') {
          ...
          // Pré-remplir la grille tarifaire
          const initialGrille = {}
          forfaits.forEach((f) => {
  ...
  if (loading) {
    return <LoadingState message="Chargement des détails de l'offre..." />
  }

  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      <PageHeader
        title="Modifier l'Offre (Fiche Unique)"
        description="Ajustez les informations générales et la grille de tarification multi-durées de votre offre."
        action={
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Card className="p-6 border border-border bg-card shadow-xs space-y-4">
                        ...
                        <td className="py-3.5 px-3 text-right">
                          {isSelected ? (
                            hasAtLeastOneActive ? (
                              ...
                              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-amber-500/10 text-amber-600 dark:text-amber-400 font-bold text-[10px]">
                                Incomplet
                              </span>
                            )
                          ) : (
          ...
          <div className="p-3.5 rounded-2xl bg-muted/40 border border-border flex items-center gap-3 mt-3">
            ...
            <div>
              <span className="text-xs font-bold text-foreground block">Stockage & Synchronisation Automatique</span>
              <p className="text-[11px] text-muted-foreground">
                Le stock disponible est calculé et synchronisé en temps réel à partir de vos profils dans <strong>Stock & Identifiants</strong>.
              </p>
            </div>
          </div>
        </Card>
        ...
        <div className="flex items-center justify-end gap-3">
          <Button type="button" variant="outline" onClick={() => navigate('/partenaire/offres')} disabled={submitting}>
            Annuler
          </Button>
          <Button type="submit" disabled={submitting} className="gap-2 font-bold">
