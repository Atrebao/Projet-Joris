...
export const DateFilter = ({ filtrerChiffreAffaire }) => {
  ...
  const updateFiltre = (periode, mois, annee, debut, fin, status = "") => {
    ...
    // Vérification si la fonction filtrerChiffreAffaire est définie
    ...
    <div className="ml-auto lg:flex gap-x-3">
      <select
        ...
        onChange={(e) => {
          ...
          setSelectPeriode(newPeriode);
          updateFiltre(newPeriode, selectMois, selectAnnee, startDate, endDate);
        }}
      ...
      {selectPeriode === "MOIS" && (
        <div className="flex items-center gap-x-3">
          <select
            value={selectMois}
            onChange={(e) => {
              const newMois = Number(e.target.value);
              setSelectMois(newMois);
              updateFiltre(selectPeriode, newMois, selectAnnee, startDate, endDate);
            }}
            className="my-2 select"
          >
          ...
          <select
            value={selectAnnee}
            onChange={(e) => {
              const newAnnee = Number(e.target.value);
              setSelectAnnee(newAnnee);
              updateFiltre(selectPeriode, selectMois, newAnnee, startDate, endDate);
            }}
            className="my-2 select"
          >
